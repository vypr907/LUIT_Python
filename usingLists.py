...
# 4) reverse the users list and assign the result to 'revUsers'.
revUsers = list(reversed(users))

assert revUsers == ['alice','kevin'], f"Expected 'revUsers' to be ['alice', 'kevin'] but got: {repr(users)}"

# 5) add the user 'melody' to users where 'bob' userd to be
users.insert(1,'melody')

assert users == ['kevin', 'melody', 'alice'], f"Expected 'users' to be ['kevin', 'melody', 'alice'] but got: {repr(users)}"

# 6) add the users 'adny', 'wanda', and 'jim' to the users list using a single command
...
# 7) slice the users lists to return the 3rd and 4th items and assign the result to 'centerUsers'
centerUsers = users[2:4]

assert centerUsers == ['alice', 'andy'], f"Expected 'centerUsers' to be ['alice', 'andy'] but got {repr(centerUsers)}"